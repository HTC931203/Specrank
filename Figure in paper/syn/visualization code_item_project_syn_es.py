...
C1_item = plt.scatter(C1[:,0],C1[:,1],s=30,c='blue',marker='*',alpha=0.3,label='C1')
C2_item = plt.scatter(C2[:,0],C2[:,1],s=50,c='red',marker='o',alpha=0.3,label='C2') 
...
'''
arrow1 = plt.arrow(x[0],y1[0],x[29]-x[0],y1[29]-y1[0],width=0.001,length_includes_head=True,head_width=0.07, head_length=0.05,color='dodgerblue',label='C1_CoeVec')
#arrow2 = plt.arrow(x[0],y2[0],x[49]-x[0],y2[49]-y2[0],width=0.001,length_includes_head=True,head_width=0.07, head_length=0.05,color='lightcoral',label='C2_CoeVec')
...
for d in range(C1.shape[0]):
    ...
    plt.scatter(x1,y1,s=30,c='blue',marker='x',alpha=0.7)    
...
#    plt.scatter(x3,y3,s=30,c='red',marker='x',alpha=0.7)
...
plt.legend([C2_item,C1_item,arrow1], ['Class 1','Class 2','Class 1 Feature Vector',],prop={'size': 12},loc='upper left')
#plt.gcf().savefig("syn_es_C2_pro.png",bbox_inches='tight', dpi=300)
plt.show()     
